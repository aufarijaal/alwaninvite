...
const fetchInvitation = async () => {
    ...
    try {
        ...
        form.value = {
            slug: data.slug,
            title: data.title || '',
            language: data.language as 'en' | 'id',
            theme_id: data.theme_id,
            published: data.published,
            bride_callname: data.bride_callname,
            bride_fullname: data.bride_fullname,
            bride_info_1: data.bride_info_1 || '',
            bride_info_2: data.bride_info_2 || '',
            bride_info_3: data.bride_info_3 || '',
            groom_callname: data.groom_callname,
            ...
            livestream_event_date: data.livestream_event_date || '',
            livestream_start_time: data.livestream_start_time || '',
            livestream_is_active: data.livestream_is_active,
            audio_id: data.audio_id,
            autoplay: data.autoplay,
            loop: data.loop,
            is_active: data.is_active,
            confetti_on_wish: (data.user_config as any)?.confetti_on_wish ?? true,
            confetti_emoji_1: (data.user_config as any)?.confetti_emoji_1 || '🙏',
            confetti_emoji_2: (data.user_config as any)?.confetti_emoji_2 || '❤️',
            wish_auto_reply: (data.user_config as any)?.wish_auto_reply || '',
            sunday_as_ahad: (data.user_config as any)?.sunday_as_ahad ?? false
        }
    } catch (err: any) {
...
const submitForm = async () => {
    ...
    try {
        ...
        try {
            ...
            const akadEvent = (form.value.events as any[]).find((e: any) => e.type === 'ceremony')
...
const loadMockData = (mockName: string) => {
    ...
    if (mockData) {
        form.value = { ...form.value, ...mockData }
    ...
    <div class="max-w-5xl mx-auto space-y-6 pb-10">
        ...
        <template v-else>
            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                <div v-if="isDev">
                    <button type="button" @click="showMockDataModal = true"
                        class="btn btn-xs btn-outline border-dashed border-warning text-warning">
                        <span class="font-mono">DEV</span> Load Mock Data
                    </button>
                </div>
            </div>

            <!-- Dev-only: Mock Data Modal -->
            <div v-if="isDev && showMockDataModal" class="modal modal-open">
                <div class="modal-box">
                    <h3 class="font-bold text-lg mb-4">Load Mock Data</h3>
                    <div class="flex flex-col gap-2">
                        <button v-for="name in mockDataNames" :key="name" type="button" @click="loadMockData(name)"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="form-control" data-field="title">
                                ...
                                <input v-model="form.title" type="text"
                                    :placeholder="t('invitation.placeholders.title')" class="input input-bordered"
                            ...
                            <div class="form-control md:col-span-2">
                                ...
                                <!-- Select Theme Button -->
                                <button v-else type="button" @click="showThemeModal = true"
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="form-control" data-field="bride_callname">
                                    <label class="label">
                                        <span class="label-text font-medium">{{ t('invitation.fields.callname') }}
                                            *</span>
                                    </label>
                                    <input v-model="form.bride_callname" type="text"
                                        :placeholder="t('invitation.placeholders.brideCallname')"
                                        class="input input-bordered"
                                        :class="{ 'input-error': errors.bride_callname }" />
                                ...
                                <div class="form-control" data-field="bride_fullname">
                                    <label class="label">
                                        <span class="label-text font-medium">{{ t('invitation.fields.fullname') }}
                                            *</span>
                                    </label>
                                    <input v-model="form.bride_fullname" type="text"
                                        :placeholder="t('invitation.placeholders.brideFullname')"
                                        class="input input-bordered"
                                        :class="{ 'input-error': errors.bride_fullname }" />
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="form-control" data-field="groom_callname">
                                    ...
                                    <input v-model="form.groom_callname" type="text"
                                        ...
                                        class="input input-bordered"
                                        :class="{ 'input-error': errors.groom_callname }" />
                                ...
                                <div class="form-control" data-field="groom_fullname">
                                    ...
                                    <input v-model="form.groom_fullname" type="text"
                                        ...
                                        class="input input-bordered"
                                        :class="{ 'input-error': errors.groom_fullname }" />
                            ...
                            <div v-for="(event, index) in form.events" :key="index"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div v-if="event.type !== 'ceremony'" class="form-control">
                                        ...
                                        <FpFormDateTimePicker v-model="event.end_time"
                                    ...
                                    <div class="form-control" :data-field="`event_location_${index}`">
                                        <label class="label">
                                            <span class="label-text font-medium">{{
                                                t('invitation.fields.locationName') }}
                                                *</span>
                                        </label>
                            ...
                            <div class="form-control">
                                <label class="label cursor-pointer justify-start gap-4">
                                    <input v-model="form.confetti_on_wish" type="checkbox"
                                        class="checkbox checkbox-primary" />
                                    <span class="label-text font-medium">{{ t('invitation.fields.confettiOnWish')
                                    }}</span>
                                </label>
                            ...
                            <div class="form-control">
                                <label class="label cursor-pointer justify-start gap-4">
                                    <input v-model="form.sunday_as_ahad" type="checkbox"
                                        class="checkbox checkbox-primary" />
                                    <span class="label-text font-medium">{{ t('invitation.fields.sundayAsAhad')
                                        }}</span>
                                </label>
                                <p class="text-xs text-base-content/50 ml-10">{{ t('invitation.helpers.sundayAsAhad') }}
                                </p>
                            </div>
                        </div>
